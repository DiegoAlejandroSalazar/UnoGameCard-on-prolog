regole :-
   ...
   % send(@finestraregole, append, text('      hai una carta dello stesso colore o stessa valore della carta al centro del tavolo;', left, font(helvetica, normal, 12))),
    send(@finestraregole, append, text('   - Altrimenti se non si ha possibilità di giocare, è obbligatorio pescare una carta;', left, font(helvetica, normal, 12))),
    send(@finestraregole, append, text('   - Il turno passa poi all\'avversario, che farà la sua mossa;', left, font(helvetica, normal, 12))),
    ...
    % Carte speciali
     ...
     send(@finestraregole, append, text('   - Le carte cambio colore permettono di sceliere il colore della prossima carta che l\'avversario dovrà giocare;', left, font(helvetica, normal, 12))),
     send(@finestraregole, append, text('   - Le carte stop permettono di bloccare il turno dell\'avversario e quindi giocare una carta in più.', left, font(helvetica, normal, 12))),

    % bottone UNO!
    send(@finestraregole, append, text('4. Bottone UNO!:', left, font(helvetica, bold, 14))),
    send(@finestraregole, append, text('   - Quando si avrà possibilità di giocare una carta si potrà cliccare il bottone UNO!;', left, font(helvetica, normal, 12))),
