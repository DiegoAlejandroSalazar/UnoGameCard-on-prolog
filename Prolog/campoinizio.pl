...
cambio_colore(Bottone) :-
    boxes_colori(Lista),
    cerca_colore(Bottone,Lista,_,Colore),
    retractall(colore(_)),
    assertz(colore(Colore)),
    writeln(Colore).


% Funzione per cercare il colore corrispondente a un bottone
cerca_colore(Bottone, [[Bottone, Colore]|_], Bottone, Colore):-!.
cerca_colore(Bottone, [_|Rest], Bottone, Colore) :-
    cerca_colore(Bottone, Rest, Bottone, Colore).
...
wait_for_color_selection(NuovoColore) :-
    ...
    send(@dialog, display, @bottoneRosso, point(70,350)),
    send(@bottoneRosso, recogniser, click_gesture(left, '', single,
                 message(@prolog,  colore_selezionato, red))),
    ...
    send(@dialog, display, @bottoneBlu, point(150,350)),
    send(@bottoneBlu, recogniser, click_gesture(left, '', single,
                 message(@prolog,  colore_selezionato, blue))),
    ...
    send(@dialog, display, @bottoneGiallo, point(70,400)),
    send(@bottoneGiallo, recogniser, click_gesture(left, '', single,
                 message(@prolog,  colore_selezionato, orange))),
    ...
    send(@dialog, display, @bottoneVerde, point(150,400)),
    send(@bottoneVerde, recogniser, click_gesture(left, '', single,
                 message(@prolog, colore_selezionato, green))),

    retractall(turno_bloccato(_)),
    assertz(turno_bloccato(si)),
    controllo_colore_selezionato,
    colore(NuovoColore).
    %sleep(5).


colore_selezionato(Colore) :-
    send(@bottoneRosso, destroy),
    send(@bottoneBlu, destroy),
    send(@bottoneGiallo, destroy),
    send(@bottoneVerde, destroy),
    retractall(turno_bloccato(_)),
